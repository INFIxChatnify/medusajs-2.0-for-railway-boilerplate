...
class MyAuthProviderService extends AbstractAuthModuleProvider {
  ...
  async validateCallback(
    ...
    try {
      ...
      const response = await axios
        .post("https://account.omplatform.com/oauth/authenticate", data)
        .then((r) => {
          if (!r.status) {
            throw new MedusaError(
  ...
  async verify_(
    ...
    const response = await axios
      .get("https://account.omplatform.com/oauth/me", {
        headers: {
  ...
  private getRedirect(clientId: string, callbackUrl: string, stateKey: string) {
    ...
    const authUrl = new URL(`https://account.omplatform.com/oauth/authorize`);
